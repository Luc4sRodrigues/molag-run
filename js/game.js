...
function checkWreck() {
	en_cars.forEach(function (e) {
		var playa_loc = playa.getBoundingClientRect(),
			half = playa_loc.width * 0.5;
		(tl = document.elementFromPoint(playa_loc.x + half, playa_loc.y + half)),
			(tr = document.elementFromPoint(
				playa_loc.x + playa_loc.width - half,
				playa_loc.y + half
			)),
			(bl = document.elementFromPoint(
				playa_loc.x + half,
				playa_loc.y + playa_loc.height - half
			)),
		...
		if (
			tl.classList.contains("en_car") ||
			tr.classList.contains("en_car") ||
			bl.classList.contains("en_car") ||
			br.classList.contains("en_car")
		) {
...
function startGame(e) {
	if (e.target.id == "instructions") {
		...
		(rot = 0), (rot_speed = 3), (laps = 0), (car_count = 0);
		laps_count.innerHTML = laps;
